...
[v1.5]
1.add timeout for all urllib.request.urlopen to try to avoid dead url link
...
#Note: The html.entities module has been renamed to html.entities in Python 3.0.
# so html.entities is only available between Python 2.3 and Python 2.7
...
gConst = {
    ...
    'defaultTimeout': 20, # default timeout seconds for urllib.request.urlopen
}
...
def decodeHtmlEntity(origHtml, decodedEncoding=""):
    ...
    # 'gt': '>',
    # 'yen': '\xa5',
    #logging.debug("html.entities.entitydefs=%s", html.entities.entitydefs);
    ...
    # 'sup': 8835,
    # 'Ntilde': 209,
    #logging.debug("html.entities.name2codepoint=%s", html.entities.name2codepoint);
    ...
    # 8195: 'emsp',
    # 8709: 'empty',
    #logging.debug("html.entities.codepoint2name=%s", html.entities.codepoint2name);
    ...
    #when use:
    #decodedEntityName = re.sub('&(?P<entityName>[a-zA-Z]{2,10});', lambda matched: unichr(html.entities.name2codepoint[matched.group("entityName")]), origHtml);
    ...
    def _nameToCodepoint(matched):
        ...
        if(entityName in html.entities.name2codepoint):
            decodedCodepoint = html.entities.name2codepoint[entityName];
...
def htmlEntityNameToCodepoint(htmlWithEntityName):
    ...
    # "&Ntilde":"&#209;",
    nameToCodepointDict = {};
    for eachName in html.entities.name2codepoint:
        fullName = "&" + eachName + ";";
        fullCodepoint = "&#" + str(html.entities.name2codepoint[eachName]) + ";";
...
def htmlEntityCodepointToName(htmlWithCodepoint):
    ...
    # "&#8709;": "&empty;",
    codepointToNameDict = {};
    for eachCodepoint in html.entities.codepoint2name:
        fullCodepoint = "&#" + str(eachCodepoint) + ";";
        fullName = "&" + html.entities.codepoint2name[eachCodepoint] + ";";
...
def initAutoHandleCookies(localCookieFileName=None):
    ...
    if(localCookieFileName):
        ...
        #gVal['cj'] = cookiejar.FileCookieJar(localCookieFileName); #NotImplementedError
        gVal['cj'] = cookiejar.LWPCookieJar(localCookieFileName); # prefer use this
        #gVal['cj'] = cookiejar.MozillaCookieJar(localCookieFileName); # second consideration
        #create cookie file
        gVal['cj'].save();
    else:
        ...
        gVal['cj'] = cookiejar.CookieJar();

    opener = urllib.request.build_opener(urllib.request.HTTPCookieProcessor(gVal['cj']));
...
def initProxy(singleProxyDict = {}):
    """Add proxy support for later urllib.request auto use this proxy
    
    Note:
    1. tmp not support username and password
    2. after this init, later urllib.request.urlopen will automatically use this proxy
    """

    proxyHandler = urllib.request.ProxyHandler(singleProxyDict);
...
def initProxyAndCookie(singleProxyDict = {}, localCookieFileName=None):
    ...
    Note:
    1. after this init, later urllib.request.urlopen will auto, use proxy, auto handle cookies
    ...
    proxyAndCookieOpener = urllib.request.build_opener(urllib.request.HTTPCookieProcessor(gVal['cj']), proxyHandler);
    #print "proxyAndCookieOpener=",proxyAndCookieOpener;
    urllib.request.install_opener(proxyAndCookieOpener);
...
################################################################################
# Network: urllib/urllib.request/http
################################################################################
...
def isFileValid(fileUrl) :
    ...
    try :
        ...
        #resp = urllib.request.urlopen(fileUrl, timeout=gConst['defaultTimeout']); # note: Python 2.6 has added timeout support.
        #some url, such as
        #http://my.csdn.net/uploads/201205/03/1336006009_2164.jpg
        #if not give user-agent, then will error: HTTP Error 403: Forbidden
        request = urllib.request.Request(fileUrl, headers={'User-Agent' : gConst['UserAgent']});
        #print "request=",request;
        resp = urllib.request.urlopen(request, timeout=gConst['defaultTimeout']);
        ...
        fileIsValid = False;
        errReason = reason;
    except urllib.request.HTTPError as code :
        fileIsValid = False;
        errReason = code;
    except :
        fileIsValid = False;
    ...
    # here type(errReason)= <class 'urllib.request.HTTPError'>, so just convert it to str
    ...
    try :
        if fileUrl :
            ...
            if(headerDict):
                ...
                resp = urllib.request.urlopen(fileUrl, timeout=gConst['defaultTimeout']);
                #print "resp=",resp;
...
def getUrlResponse(url, postDict={}, headerDict={}, timeout=0, useGzip=False, postDataDelimiter="&") :
    ...
    if (postDict) :
        ...
        #req = urllib.request.Request(url, postData);#python2.0

        # print (urllib.parse.urlencode(postDict))
        # print ("\r\n")
        # print (urllib.parse.urlencode(postDict).encode(encoding='UTF8'))  外层多了 b'{0}'
        # print ("\r\n")

        req = urllib.request.Request(url,data=urllib.parse.urlencode(postDict).encode(encoding='UTF8') );#没有 .encode(encoding='UTF8') 不行
...
def getZipcodeFromLocation(locationStr):
    ...
    if(not gotZipCode):
        ...
        encodedPara = urllib.parse.urlencode(paraDict);
